...
struct UpbExtensionTrait<T> {
  ...
  static absl::Status Set(Msg message, const Id& id, T&& value) {
    ...
    return hpb::internal::MoveExtension(hpb::interop::upb::GetMessage(message),
                                        hpb::interop::upb::GetArena(message),
                                        id.mini_table_ext(),
                                        hpb::interop::upb::GetMessage(&local),
                                        hpb::interop::upb::GetArena(&local));
  }

  template <typename Msg, typename Id>
  static absl::Status Set(Msg message, const Id& id, hpb::Ptr<const T> value) {
...
template <typename T, typename Extension, typename Input>
auto SetExtension(
