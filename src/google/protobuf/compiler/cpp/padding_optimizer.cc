...
static void OptimizeLayoutHelper(std::vector<const FieldDescriptor*>* fields,
  ...
  for (int f = 0; f < kMaxFamily; f++) {
    ...
    for (std::size_t i = 0; i < aligned_to_1[f].size(); i += 4) {
      FieldGroup field_group;
      for (std::size_t j = i; j < aligned_to_1[f].size() && j < i + 4; ++j) {
    ...
    for (std::size_t i = 0; i < aligned_to_4[f].size(); i += 2) {
      FieldGroup field_group;
      for (std::size_t j = i; j < aligned_to_4[f].size() && j < i + 2; ++j) {
  ...
  fields->clear();
  for (int f = 0; f < kMaxFamily; ++f) {
    for (std::size_t i = 0; i < aligned_to_8[f].size(); ++i) {
