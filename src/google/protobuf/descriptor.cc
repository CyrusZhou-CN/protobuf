...
class FlatAllocatorImpl {
 public:
  ...
  void PlanFieldNames(size_t parent_scope_size, const absl::string_view name,
    ...
    constexpr int kNullChar = 1;
    size_t total_bytes = kIndexSize + name.size() + kNullChar;
    ...
    if (opt_json_name == nullptr) {
      switch (GetFieldNameCase(name)) {
        ...
        case FieldNameCase::kSnakeCase: {
          ...
          return PlanArray<char>(total_bytes + CamelCaseSize(name) + kNullChar);
        }
        default:
          break;
      }
    ...
    // lowercase
    total_bytes += name.size() + kNullChar;
    // camelcase
    total_bytes += CamelCaseSize(name) + kNullChar;
    // json_name
