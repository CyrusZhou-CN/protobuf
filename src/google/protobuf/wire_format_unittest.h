...
TYPED_TEST_P(WireFormatTest, CheckRequiredFields) {
  const char* ptr;
  typename TestFixture::TestRequired msg;
  msg.set_a(1);
  std::string serialized = msg.SerializePartialAsString();

  // Don't set flag to test required fields
  {
    typename TestFixture::TestRequired dest;
    google::protobuf::internal::ParseContext ctx(100, false, &ptr, serialized);
    ptr = WireFormat::_InternalParse(&dest, ptr, &ctx);
    EXPECT_FALSE(ctx.missing_required_fields());
  }

  // Set flag to test required fields
  {
    typename TestFixture::TestRequired dest;
    google::protobuf::internal::ParseContext ctx(100, false, &ptr, serialized);
    ctx.SetCheckRequiredFields(true);
    ptr = WireFormat::_InternalParse(&dest, ptr, &ctx);
    EXPECT_TRUE(ctx.missing_required_fields());
  }

  // Message not missing required fields.
  msg.set_b(1);
  msg.set_c(1);
  serialized = msg.SerializePartialAsString();
  {
    typename TestFixture::TestRequired dest;
    google::protobuf::internal::ParseContext ctx(100, false, &ptr, serialized);
    ctx.SetCheckRequiredFields(true);
...
TYPED_TEST_P(WireFormatTest, MessageSetRecursionLimit) {
  auto extension_id =
      TestFixture::TestMessageSetExtension1::message_set_extension;
  std::string data;
  {
    typename TestFixture::NestedTestMessageSetContainer original;

    original.mutable_lazy_child()
        ->mutable_container()
        ->mutable_message_set()
        ->MutableExtension(extension_id)
        ->mutable_lazy_payload()
        ->mutable_child()
        ->mutable_child()
        ->set_b(-1);
  ...
  input_stream.SetRecursionLimit(6);
  typename TestFixture::NestedTestMessageSetContainer parsed;
  ASSERT_FALSE(parsed.ParseFromCodedStream(&input_stream));

  const auto& mset = parsed.lazy_child().container().message_set();
  ASSERT_TRUE(mset.HasExtension(extension_id));
  auto& m = mset.GetExtension(extension_id);
  EXPECT_TRUE(m.has_lazy_payload());
  EXPECT_TRUE(m.lazy_payload().has_child());
  EXPECT_FALSE(m.lazy_payload().child().has_child());
}
...
REGISTER_TYPED_TEST_SUITE_P(
    ...
    UnknownFieldRecursionLimit, ZigZag, CheckRequiredFields,
    MessageSetRecursionLimit, MessageSetLargeTypeId);
...
class WireFormatInvalidInputTest : public testing::Test,
                                   protected TestUtil::TestUtilTraits<T> {
 protected:
  using TestAllTypes = TestUtil::TestUtilTraits<T>::TestAllTypes;
