...
#if !defined(__GNUC__) || defined(__clang__) || PROTOBUF_GNUC_MIN(8, 2)
...
TEST(MapTest, StaticTypeInfoMatchesDynamicOne) {
  TestAllKeyValueTypes(KeyTypes(), ValueTypes(), [](auto key, auto value) {
    ...
    const auto dyn_type_info = internal::UntypedMapBase::GetTypeInfoDynamic(
        static_cast<UntypedMapBase::TypeKind>(type_info.key_type),
        static_cast<UntypedMapBase::TypeKind>(type_info.value_type),
        value_prototype);
