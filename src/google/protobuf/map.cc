...
void UntypedMapBase::ClearTableImpl(bool reset) {
  ...
  if (arena_ == nullptr) {
    ...
    const auto dispatch_key = [&](auto value_handler) {
      if (static_cast<TypeKind>(type_info_.key_type) < TypeKind::kString) {
        loop(value_handler);
      } else if (static_cast<TypeKind>(type_info_.key_type) ==
                 TypeKind::kString) {
        loop([=](NodeBase* node) {
    ...
    if (static_cast<TypeKind>(type_info_.value_type) < TypeKind::kString) {
      dispatch_key([](NodeBase*) {});
    } else if (static_cast<TypeKind>(type_info_.value_type) ==
               TypeKind::kString) {
