...
class Wire {
 public:
  ...
  template <typename Sink>
  friend void AbslStringify(Sink& sink, Wire&& wire) {
    sink.Append(std::move(wire.buf_));
  }
...
Wire LengthPrefixed(std::string data);
...
// format, although it will typically be composed with additional elements using
// Wire(...). These are really just meant as lightweight helpers to make
...
inline Wire LengthPrefixedField(uint32_t fieldnum, std::string content) {
  return Wire(Tag(fieldnum, WireType::kLengthPrefixed),
              LengthPrefixed(std::move(content)));
}
inline Wire DelimitedField(uint32_t fieldnum, std::string content) {
