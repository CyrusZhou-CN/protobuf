...
static bool rm(upb_table* t, lookupkey_t key, upb_value* val, uint32_t hash,
  ...
  if (eql(chain->key, key)) {
    ...
    if (chain->next) {
      ...
      *chain = *move;
      move->key = upb_key_empty(); /* Make the slot empty. */
    } else {
      chain->key = upb_key_empty(); /* Make the slot empty. */
    }
    return true;
  } else {
    ...
    if (chain->next) {
      ...
      rm->key = upb_key_empty(); /* Make the slot empty. */
      chain->next = rm->next;
      return true;
    } else {
  ...
  if (key < t->array_size) {
    ...
    if (isfull(&t->t)) {
      ...
      for (i = begin(&t->t); i < upb_table_size(&t->t); i = next(&t->t, i)) {
        ...
        upb_value v = e->val;
        insert(&new_table, intkey(e->key.num), e->key, v, inthash(e->key),
               &inthash, &inteql);
      }
