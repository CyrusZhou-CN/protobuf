...
static const upb_tabent* upb_getentry(const upb_table* t, uint32_t hash) {
  return t->entries + (hash & (upb_table_size(t) ? upb_table_size(t) - 1 : 0));
}
...
static bool init(upb_table* t, uint8_t log2size, upb_Arena* a) {
  t->count = 0;
...
static size_t _upb_need_entries(size_t expected_size) {
...
bool upb_strtable_init(upb_strtable* t, size_t expected_size, upb_Arena* a) {
  int size_lg2 = upb_Log2Ceiling(_upb_need_entries(expected_size));
...
bool upb_strtable_insert(upb_strtable* t, const char* k, size_t len,
  ...
  if (isfull(&t->t)) {
    ...
    if (!upb_strtable_resize(t, _upb_popcnt32(t->t.mask) + 1, a)) {
      return false;
    }
  }
...
bool upb_inttable_insert(upb_inttable* t, uintptr_t key, upb_value val,
  ...
  if (key < t->array_size) {
    ...
    if (isfull(&t->t)) {
      ...
      if (!init(&new_table, _upb_popcnt32(t->t.mask) + 1, a)) {
        return false;
      }
...
void upb_inttable_compact(upb_inttable* t, upb_Arena* a) {
...

  {
    ...
    size_t hash_size = hash_count ? _upb_need_entries(hash_count) : 0;
