...
class AnyTest(unittest.TestCase):
  ...
  def testAnyMessage(self):
    ...
    self.assertEqual(msg.value.UnpackAs(unittest_pb2.TestAllTypes), all_types)

    # Fails to unpack to a different type.
    self.assertFalse(msg.value.Unpack(msg))

    with self.assertRaises(TypeError) as catcher:
      msg.value.UnpackAs(well_known_types_test_pb2.TestAny)
